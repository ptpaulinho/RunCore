...
def tool(fn: Callable | None = None, *, name: str | None = None) -> Callable:
    ...
    def _decorate(target: Callable) -> Callable:
        ...
        def _wrapper(*args, **kwargs):
            cap = _ctx.current()
            if cap is None:
                from runcore.replay import lookup_tool
                hit, recorded = lookup_tool(tool_name, _bind_arguments(target, args, kwargs))
                return recorded if hit else target(*args, **kwargs)
            ...
            from runcore.replay import lookup_tool
            hit, recorded = lookup_tool(tool_name, arguments)
            if hit:  # inside runcore.replay.recorded_tools(): serve the recording, run nothing
                cap.record_tool(tool_name, arguments, recorded, True, duration_ms=0.0,
                                metadata={"replayed": True}, skip_guard=True)
                return recorded
