"""RunCore CLI — the reliability gate for AI agents."""
...
app = typer.Typer(
    name="runcore",
    help="RunCore — reliability gate for AI agents. Catch cost/quality regressions in CI, fix waste at runtime, verify any agent.",
...
def serve(
    ...
    if importlib.util.find_spec("runcore.server") is None:
        console.print("The dashboard is part of RunCore Cloud: https://runcore.onrender.com "
                      "(upload runcore_trace.json there, or set RUNCORE_API_KEY).")
        raise typer.Exit(1)
...
def certify(
    ...
    try:
        ...
        console.print("Official certification runs on RunCore Cloud — hidden, rotating exam and a signed "
                      "certificate anyone can verify: https://runcore.onrender.com/app/certify\n"
                      "Locally, gate regressions with `runcore ci` and test changes with `runcore replay`.")
        raise typer.Exit(1)
...
@app.command()
def replay(
    traces: list[str] = typer.Argument(..., help="Trace files (.json/.jsonl) or directories"),
    model: str = typer.Option(..., "--model", "-m", help="Candidate as provider:model, e.g. groq:llama-3.3-70b-versatile"),
    judge: Optional[str] = typer.Option(None, "--judge", help="LLM judge for text answers, provider:model"),
    base_url: Optional[str] = typer.Option(None, "--base-url", help="Custom OpenAI-compatible endpoint"),
    max_steps: Optional[int] = typer.Option(None, "--max-steps", help="Stop after N steps"),
    output: Optional[str] = typer.Option(None, "--output", "-o", help="Save JSON report"),
    fail_below: float = typer.Option(0.0, "--fail-below", help="Exit 1 if agreement < this (0-1), for CI"),
) -> None:
    """Replay recorded agent runs against another model — before you switch."""
    from runcore import replay as rp

    try:
        loaded = rp.load_traces(traces)
        rp.parse_target(model)
    except (OSError, ValueError, rp.ReplayError) as exc:
        console.print(f"[red]{exc}[/red]")
        raise typer.Exit(1)
    if not loaded:
        console.print("[red]No traces found.[/red]")
        raise typer.Exit(1)

    marks = {"same": "[green]=[/green]", "similar": "[green]~[/green]", "equivalent": "[green]≈[/green]",
             "error": "[yellow]![/yellow]"}
    report = rp.replay(loaded, model, judge_target=judge, base_url=base_url, max_steps=max_steps,
                       on_step=lambda st: console.print(marks.get(st.status, "[red]x[/red]"), end=""))
    console.print()
    sm = report.summary()
    if not sm["compared"] and not sm["errors"]:
        console.print("[yellow]No recorded LLM steps. Capture with runcore >= 0.12 "
                      "(import runcore.auto) so requests are recorded.[/yellow]")
        raise typer.Exit(1)

    t = Table(box=box.SIMPLE, header_style="bold dim", title=f"Replay → {model}")
    t.add_column("Metric", style="cyan"); t.add_column("Original", justify="right"); t.add_column("Candidate", justify="right")
    orig_models = ", ".join(sorted({st.original_model for st in report.steps})) or "?"
    t.add_row("model", orig_models, model)
    cand = "unknown price" if sm["candidate_cost_usd"] is None else f"${sm['candidate_cost_usd']:.6f}"
    chg = "" if sm["cost_change_pct"] is None else f" ({sm['cost_change_pct']:+.1f}%)"
    orig = "unknown price" if sm["original_cost_usd"] is None else f"${sm['original_cost_usd']:.6f}"
    t.add_row("cost", orig, cand + chg)
    t.add_row("latency", f"{sm['original_latency_ms']:.0f} ms", f"{sm['candidate_latency_ms']:.0f} ms")
    t.add_row("agreement", "", f"{sm['agreement']*100:.1f}% of {sm['compared']} steps")
    t.add_row("tool decisions changed", "", str(sm["tool_decisions_changed"]))
    if sm["errors"]:
        t.add_row("errors", "", str(sm["errors"]))
    console.print(t)

    for st in [s for s in report.steps if s.status not in ("same", "similar", "equivalent")][:5]:
        what = st.error or f"{st.status}: {json.dumps(st.original.get('tool_calls') or st.original.get('text', '')[:80])[:120]}" \
                            f" → {json.dumps(st.candidate.get('tool_calls') or st.candidate.get('text', '')[:80])[:120]}"
        console.print(f"  [dim]{st.trace_id[:8]} step {st.step}[/dim] {what}")

    if sm["unverified_text"]:
        console.print(f"  [yellow]{sm['unverified_text']} text answer(s) reworded — add --judge provider:model to verify them.[/yellow]")
    colors = {"safe": "green", "review": "yellow", "unsafe": "red", "no_data": "yellow"}
    console.print(Panel(f"Verdict: [bold]{sm['verdict'].upper()}[/bold]", border_style=colors[sm["verdict"]]))
    if output:
        Path(output).write_text(json.dumps(report.to_dict(), indent=2, default=str))
        console.print(f"[dim]Report → {output}[/dim]")
    if fail_below and sm["agreement"] < fail_below:
        raise typer.Exit(1)
...
def fix(
    traces: list[str] = typer.Argument(..., help="Trace files (.json/.jsonl) or directories"),
    current: str = typer.Option(..., "--current", "-c", help="Model your agent uses now, provider:model"),
    try_: str = typer.Option("", "--try", "-t", help="Comma-separated candidate models, provider:model"),
    judge: Optional[str] = typer.Option(None, "--judge", help="LLM judge for text answers, provider:model"),
    base_url: Optional[str] = typer.Option(None, "--base-url", help="Custom OpenAI-compatible endpoint"),
    max_steps: Optional[int] = typer.Option(None, "--max-steps", help="Stop each replay after N steps"),
    write_policy: str = typer.Option("runcore.fix.json", "--write-policy", help="Where to write the verified policy ('' = don't)"),
    markdown: Optional[str] = typer.Option(None, "--markdown", help="Write a PR/CI comment (Markdown)"),
) -> None:
    """Find cheaper configurations, verify each by replay, write the ones that pass."""
    from runcore import fix as fx
    from runcore import replay as rp

    try:
        loaded = rp.load_traces(traces)
        candidates = [c.strip() for c in try_.split(",") if c.strip()]
        for t in [current, *candidates]:
            rp.parse_target(t)
    except (OSError, ValueError, rp.ReplayError) as exc:
        console.print(f"[red]{exc}[/red]")
        raise typer.Exit(1)
    ...
    with console.status("Replaying your runs with each candidate fix…"):
        report = fx.find_fixes(loaded, current, candidates, judge_target=judge, base_url=base_url, max_steps=max_steps)
    if not report.fixes:
        console.print("[yellow]Nothing to try: pass --try provider:model (and every offered tool is already used).[/yellow]")
        raise typer.Exit(0)

    t = Table(box=box.SIMPLE, header_style="bold dim", title=f"Verified fixes — baseline {current}")
    t.add_column("Fix", style="cyan"); t.add_column("Result"); t.add_column("Agreement", justify="right")
    t.add_column("Cost", justify="right"); t.add_column("Input tokens", justify="right")
    pct = lambda v: "n/a" if v is None else f"{v:+.1f}%"  # noqa: E731
    for f in report.fixes:
        t.add_row(f.title, "[green]✓ verified[/green]" if f.verified else f"[red]✗ {f.verdict}[/red]",
                  f"{f.agreement * 100:.0f}% of {f.steps}", pct(f.cost_change_pct), pct(f.input_tokens_change_pct))
    console.print(t)

    policy = report.policy()
    if markdown:
        Path(markdown).write_text(report.markdown())
        console.print(f"[dim]PR comment → {markdown}[/dim]")
    if not policy["fixes"]:
        console.print(Panel("No change passed verification — keep your current configuration.", border_style="yellow"))
        raise typer.Exit(0)
    if write_policy:
        Path(write_policy).write_text(json.dumps(policy, indent=2))
    console.print(Panel("\n".join("✓ " + f for f in policy["fixes"]) +
                        (f"\n\nWritten to {write_policy} — applied automatically by `import runcore.auto`."
                         if write_policy else ""),
                        title="[bold green]Verified — safe to apply[/bold green]", border_style="green"))


@app.command()
def verify(
    certificate: str = typer.Argument(..., help="Signed certificate JSON (download it from /verify/<id>)"),
    key: Optional[str] = typer.Option(None, "--key", help="Issuer public key (base64). Default: fetch from --url"),
    url: str = typer.Option("https://runcore.onrender.com", "--url", help="RunCore issuer to fetch the public key from"),
) -> None:
    """Verify a RunCore certificate: issued by RunCore, unmodified, not expired."""
    from runcore import attest
    try:
        att = json.loads(Path(certificate).read_text())
        pub = key or attest.fetch_public_key(url)
    except (OSError, ValueError, KeyError) as exc:
        console.print(f"[red]{exc}[/red]")
        raise typer.Exit(2)
    res = attest.verify(att, pub)
    p = res.get("payload") or {}
    subj = p.get("subject") or {}
    if p:
        console.print(f"  {subj.get('company', '?')} — {subj.get('product') or subj.get('model', '?')}: "
                      f"grade [bold]{p.get('grade')}[/bold] ({p.get('score')}/100), "
                      f"issued {str(p.get('issued_at', ''))[:10]}, expires {str(p.get('expires_at', ''))[:10]}")
    color = "green" if res["valid"] else "red"
    console.print(Panel(f"{res['status'].upper()}: {res['reason']}", border_style=color))
